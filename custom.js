module.exports = async ({ api, event }) => {
  const logger = require('./main/utility/logs.js');
      ...
      const resp = await axios.get(`https://quotes.iyot.plus/motiv`);
        const text = String(resp.data.quoteText);
        const author = String(resp.data.quoteAuthor);
        const mes = `👤Author: ${author}\n📖${text}\n\n`;
